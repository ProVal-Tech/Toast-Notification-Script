    ...
    switch ($Type) {
        'ToastReboot' {
            try {
                ...
                $ackFile = '{0}\ToastRebootAcknowledged.txt' -f $Path
                $ackLine = 'echo %date% %time%> {0}{1}{0}' -f [char]34, $ackFile
                $rebootLine = 'shutdown /r /t 0 /f /d p:0:0 /c {0}Scheduled System Restart{0}' -f [char]34
                $scriptContent = '{0}{1}{2}{1}{3}' -f $ackLine, [Environment]::NewLine, $clearLine, $rebootLine
...
function Show-ToastNotification {
    ...
    .DESCRIPTION
        ...
        The notification is built as a ToastNotification object rather than passing raw XML to Show(),
        so it can carry a Tag/Group and be marked ExpiresOnReboot. Without ExpiresOnReboot the entry
        is written to wpndatabase.db and restored into the Notification Center at the next sign in.

        A short de-duplication window suppresses the toast when another engine instance has already
        displayed the same notification. Task Scheduler starts one instance per interactive session
        when the task principal is a group, so the engine can legitimately run twice within seconds.
    .PARAMETER DedupeSeconds
        Seconds during which a second engine instance suppresses its own toast. 0 disables the check.
    .PARAMETER RetentionHours
        Hours after which Windows drops the notification from the Notification Center on its own.
    .EXAMPLE
        Show-ToastNotification
        Displays the toast notification.
    .EXAMPLE
        Show-ToastNotification -DedupeSeconds 0
        Displays the toast notification without duplicate suppression.
    #>
    [CmdletBinding()]
    [OutputType([System.Void])]
    param (
    ...
    try {
        ...
        #region identity
        $identity = if ($Config) { [System.IO.Path]::GetFileNameWithoutExtension($Config) } else { 'ToastNotification' }
        $identity = $identity -replace '[^A-Za-z0-9]', ''
        if ([String]::IsNullOrWhiteSpace($identity)) { $identity = 'ToastNotification' }
        if ($identity.Length -gt 60) { $identity = $identity.Substring(0, 60) }
        Write-ToastLog -Message ('Toast identity resolved to {0}' -f $identity)
        #endregion

        #region duplicate suppression
        $stampPath = '{0}\ToastShown-{1}.txt' -f $env:LOCALAPPDATA, $identity
        if ($DedupeSeconds -gt 0) {
            $mutexName = 'Global\ToastShow-{0}-{1}' -f $identity, ($env:USERNAME -replace '[^A-Za-z0-9]', '')
            ...
            if (Test-Path -Path $stampPath) {
                $lastShown = [DateTime]::MinValue
                $rawStamp = [String](Get-Content -Path $stampPath -TotalCount 1 -ErrorAction SilentlyContinue)
                if ([DateTime]::TryParse($rawStamp, [ref]$lastShown)) {
                    $secondsSince = ((Get-Date) - $lastShown).TotalSeconds
                    if (($secondsSince -ge 0) -and ($secondsSince -lt $DedupeSeconds)) {
                        Write-ToastLog -Level Warn -Message ('Another instance displayed this notification {0} seconds ago. Suppressing the duplicate toast' -f [Math]::Round($secondsSince))
                        Write-Information -MessageData 'Duplicate toast suppressed' -InformationAction Continue
                        return
                    }
                }
            }
        ...
        #region build the notification
        $toastXml = New-Object -TypeName Windows.Data.Xml.Dom.XmlDocument
        $toastXml.LoadXml($Toast.OuterXml)

        $toastNotification = [Windows.UI.Notifications.ToastNotification]::new($toastXml)
        $toastNotification.Tag = $identity
        $toastNotification.Group = $identity

        #Windows 10 1903 / build 18362 and later.
        if ([Environment]::OSVersion.Version.Build -ge 18362) {
            try {
                $toastNotification.ExpiresOnReboot = $true
                Write-ToastLog -Message 'ExpiresOnReboot set to True. The notification will not survive a restart'
            } catch {
                Write-ToastLog -Level Warn -Message ('Failed to set ExpiresOnReboot. Reason: {0}' -f $_.Exception.Message)
            }
        } else {
            Write-ToastLog -Level Warn -Message ('ExpiresOnReboot is unavailable on build {0}. Falling back to ExpirationTime only' -f [Environment]::OSVersion.Version.Build)
        }

        if ($RetentionHours -gt 0) {
            try {
                $toastNotification.ExpirationTime = [DateTimeOffset]::Now.AddHours($RetentionHours)
                Write-ToastLog -Message ('ExpirationTime set to {0}' -f $toastNotification.ExpirationTime)
            } catch {
                Write-ToastLog -Level Warn -Message ('Failed to set ExpirationTime. Reason: {0}' -f $_.Exception.Message)
            }
        }
        #endregion

        #region clear stale copies then show
        try {
            [Windows.UI.Notifications.ToastNotificationManager]::History.Remove($identity, $identity, $App)
            Write-ToastLog -Message ('Removed any earlier copy of this notification for app {0}' -f $App)
        } catch {
            Write-ToastLog -Message 'No earlier copy of this notification was present in the Notification Center'
        }

        [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier($App).Show($toastNotification)

        try {
            Set-Content -Path $stampPath -Value ((Get-Date).ToString('o')) -Force -ErrorAction Stop
        } catch {
            Write-ToastLog -Level Warn -Message ('Failed to write the suppression stamp at {0}. Reason: {1}' -f $stampPath, $_.Exception.Message)
        }

        Write-ToastLog -Message ('All good. Toast notification was displayed. App: {0}, Tag/Group: {1}' -f $App, $identity)
        ...
        Write-ToastLog -Message ('Reason: {0}' -f $_.Exception.Message) -Level Error
        ...
        if ($mutexHeld -and $mutex) { try { $mutex.ReleaseMutex() } catch { $null } }
        if ($mutex) { try { $mutex.Dispose() } catch { $null } }
    }
}
...
#region check previous reboot by the notification
$ackWindowMinutes = 15
$ackFile = '{0}\ToastRebootAcknowledged.txt' -f $customScriptsPath

if (Test-Path -Path $ackFile) {
    $ackAgeMinutes = ((Get-Date) - (Get-Item -Path $ackFile).LastWriteTime).TotalMinutes

    if ($ackAgeMinutes -lt 0) {
        Write-ToastLog -Level Warn -Message ('Acknowledgement stamp is dated {0} minutes ahead of the current time. The system clock has stepped backward. Treating the stamp as current' -f [Math]::Abs([Math]::Round($ackAgeMinutes)))
        $ackAgeMinutes = 0
    }

    if ($ackAgeMinutes -lt $ackWindowMinutes) {
        Write-ToastLog -Level Warn -Message ('User acknowledged this notification {0} minutes ago and a restart is in progress. Suppressing the toast' -f [Math]::Round($ackAgeMinutes))
        exit 0
    }

    Write-ToastLog -Message ('Acknowledgement stamp is {0} minutes old and outside the {1} minute window. Removing it and continuing' -f [Math]::Round($ackAgeMinutes), $ackWindowMinutes)
    Remove-Item -Path $ackFile -Force -ErrorAction SilentlyContinue
}
#endregion
